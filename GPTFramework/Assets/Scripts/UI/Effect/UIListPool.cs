...
namespace UnityEngine.UI
{
    ...
    public static class UIListPool<T>
	{
		...
		/// </summary>
		private static readonly UIObjectPool<List<T>> listPool = new UIObjectPool<List<T>>(null, l => l.Clear()); // Pool de objetos para evitar asignaciones
		#endregion
	...
	public class UIObjectPool<T> where T : new()
	{
		...
		#region Constructor
		/// <summary>
		/// <para>Constructor de <see cref="UIObjectPool{T}"/></para>
		/// </summary>
		/// <param name="TactionOnGet"></param>
		/// <param name="TactionOnRelease"></param>
		public UIObjectPool(UnityAction<T> TactionOnGet, UnityAction<T> TactionOnRelease)// Constructor de UIEffectObjectPool
		{
		...
		public void Release(T item)// Lanza la lista de la pool
		{
			if (stack.Count > 0 && ReferenceEquals(stack.Peek(), item))
                Debug.LogError("Error Interno. Intentando destruir el objeto que ya se ha lanzado de la piscina.");
